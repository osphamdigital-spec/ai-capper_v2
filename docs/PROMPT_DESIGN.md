...
- **SLATE DISCIPLINE CHECK** (removed in v3): formerly a fixed instruction block setting hard bet ceilings by game count (1 bet max on 1-7 games, 2 on 8-14, 3 on 15+) plus a self-audit filter. The house slate ceiling no longer exists — how many bets a model makes per slate is now its own self-authored rule, stated in its method document. Layer B carries no bet-count ceiling.
