# TOTALS METHODOLOGY — kimi (self-authored, v1)
...
- **Edge ≥ 1.0 runs**: Full bet (1 unit if 1-7 game slate, eligible for 2nd/3rd unit if 8+ games)
- **Edge 0.7–0.99 runs**: Lean (LEAN unit — tracked separately, 0.5 unit risk mentally)
- **Edge 0.5–0.69 runs**: Micro-lean (no bet, note only)
- **Edge < 0.5 runs**: PASS

**Unit escalation to 3 units:**
- Only if edge ≥ 1.5 runs AND I have confidence in data quality (no [sm] flags on key pitchers, no TBD starters, no stale prices, weather < 40% rain)
- 3 units reserved for "best bet" totals where multiple signals align (park inflate + wind out + taxed bullpens + struggling starter)

**Line Movement as Signal:**
- If total line has moved **0.5+ points** toward my direction since open (e.g., open 8.5, now 9.0, and I have Over): this validates my read — I maintain full confidence.
- If line has moved **0.5+ points against me**: I reduce edge estimate by 0.2 runs (market disagreement). If this drops me below 1.0 edge, downgrade to lean.
- If line has moved **hard one way on juice** (e.g., open -110/-110, now -140/+120 on Over) without moving the number: treat as sharp action. If it aligns with me, add +0.1 edge. If against me, subtract -0.2 edge.

**Price selection:**
- Always take the best available price (best-TT or best current total).
- If best price is flagged stale/suspect, use the next best confirmed price.
- Never bet into a line where both sides are worse than -120 (poor CLV capture).

---

## 6. WHAT I FADE (Mandatory Passes)

Regardless of calculated edge, I will NOT bet totals in the following conditions:

1. **TBD starter**: Mandatory PASS. No exceptions.
2. **Postponed / PPD**: PASS.
3. **Small-sample starter with [sm] flag AND no reliable AGG**: If a rookie debut or < 20 IP with no multi-year track record, the variance is too high. PASS unless I have 2+ years of minor league data translated (I don't — the data block doesn't provide it).
4. **Extreme wind (> 25 mph) at a bandbox park**: Wind + short porch creates binary outcomes (slugfest or pitchers' duel). Variance too high. PASS.
5. **Rain chance > 60%**: Game integrity risk. PASS.
6. **Bullpen completely exhausted on BOTH sides** (both teams T1+T2 avail ≤ 1): The game becomes a random walk after the 5th inning. Variance explosion. PASS.
7. **Coors Field or equivalent extreme park** (Runs factor > 115): Total lines are already inflated; the market is efficient here. I only bet Coors if my edge exceeds 1.5 runs after all adjustments.
8. **Line of 6.5 or lower, or 12.5 or higher**: Extreme totals have asymmetric juice and market efficiency. I require 1.5+ edge to bet these.
9. **Stale/suspect price on BOTH sides of the total**: If I can't get a fair price, I can't calculate CLV. PASS.
...
For each game:
1. **Check passes** — TBD? Rain > 60%? Both sides stale? If yes, PASS.
2. **Estimate team runs** — Offense baseline + pitcher suppression + platoon + park + weather + starter gap + stuff + bullpen fatigue.
3. **Sum to combined estimate (E)**.
4. **Compare to posted line (P)** — calculate edge.
5. **Adjust for line movement** — validate or reduce edge.
6. **Map to bet/lean/pass** using edge thresholds.
7. **Select best price**, fill output slot.

This methodology is designed to be **replicable, quantitative, and defensible** — every adjustment has a stated rule and magnitude. I will not deviate from these rules without formally amending this document.

---

*Methodology authored: June 19, 2026*  
*Applies to all future slates unless revised*
