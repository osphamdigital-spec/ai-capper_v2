...
2026-06-24: Lineup watcher killed by parent process — _fired.json never created, all 06-24 picks graded at original stake (HOLD). Root cause: run_daily.py --with-picks was running as a background PowerShell task spawned by the Claude Code agent session. The watcher was launched as a child of that task via run_step() (subprocess.run, blocking). When the background task exited (session ended), the watcher was killed before any MLB lineups confirmed. No data loss — confirm-check absence = HOLD is the correct fallback per the grader spec, so all picks were graded at original stake. Fix applied 2026-06-25: (1) Replaced _confirm_run_watcher() (20s countdown/prompt) with _spawn_lineup_watcher() — uses subprocess.Popen with creationflags=subprocess.CREATE_NEW_CONSOLE on Windows. The watcher gets its own console window and is fully detached (no .wait(), no .communicate()). run_daily.py prints the PID and continues. Non-fatal: if Popen fails, prints WARNING and continues. (2) Added _check_stale_watcher() called at the top of the --with-picks flow — if _watch.json exists, heartbeat absent or >5 min old, and _fired.json incomplete, prints a WARNING with the manual launch command. (3) run_lineup_watcher.py now writes daily/{sport}/{date}/_watcher_heartbeat.json at the top of every poll tick (atomic tmp-then-rename) with {pid, last_tick, date, tick}. Rule going forward: never rely on a watcher process whose lifetime is shared with an agent session or background task. The watcher must own its own terminal.

---

2026-06-23: House slate ceiling fully removed (v3) — code, prompt text, and docs. v3's design is that there is NO house slate ceiling: how many bets a model makes per slate is its own self-authored rule in its method document (Layer B carries only the edge gate, unit map, and data-integrity rules). An audit found the removed rule still lingering in three forms: (a) LIVE enforcement in query_model.py — `_slate_ceiling()` + Guard 5 in `_apply_confirm_guards()` still capped confirm-check UPGRADEs by game count (1/2/3 for ≤7 / 8-14 / 15+ games); (b) LIVE prompt text in build_prompt.py (the pre-submit reminder said "apply the edge gate, slate ceiling, and 1u/3u rule"); (c) stale doc references in CLAUDE.md, AI_CAPPER_PROJECT_INSTRUCTIONS.md, PROMPT_DESIGN.md, and POSTMORTEM_ANALYSIS_ROUTINE.md (the last listed "slate ceiling" among off-limits FIXED competition rules — logically wrong post-v3, since bets-per-slate is now a valid self-authored method element). Fixes: removed `_slate_ceiling()` and Guard 5; also removed the two helpers `_total_wagered_units()` and `_n_slate_games()` which existed solely for Guard 5 and were referenced nowhere else (orphaned-helper removal — leaving them would be dead code that still said "ceiling"). Reworded the build_prompt.py reminder to "your own max bets per slate (per your method document)". Stripped/corrected the doc refs; the POSTMORTEM routine off-limits list now reads edge gate / 1u-3u denominations / best-bet designation / output format / data-integrity, with an explicit note that bets-per-slate is NOT off-limits. **BEHAVIOR CHANGE:** confirm-check UPGRADEs are no longer capped by game-count ceiling — an UPGRADE can now raise stake regardless of how many Run-1 bets already exist, bounded only by the 3-unit stake cap (Guard 3, unchanged) and the strict-increase sanity check (Guard 4, unchanged). The 3-unit STAKE cap is NOT a slate ceiling and was deliberately kept everywhere ("3 units is the ceiling" = highest unit denomination, still valid). NOTE: this supersedes the 2026-06-21 confirm-check-watcher design note item (5) about "Guard 5 (slate ceiling) applies to UPGRADE only" — Guard 5 no longer exists. Dead `if False:` block in build_prompt.py (old SLATE DISCIPLINE CHECK text) left untouched by design — it is unreachable and serves as historical reference.

---

2026-06-22: Confirm-check parser bracket-format mismatch — every block auto-HELD on the 2026-06-22 slate (17 API calls, 0 CANCEL/DOWNGRADE/UPGRADE ever possible). Root cause was a format contract mismatch, NOT the models failing to echo the gid (they echoed it correctly — verified in the saved raw_response). build_prompt.py:3164 emits the required-response header as a SINGLE combined bracket: `PICK: {raw}  [market:{market} gid:{gid}]`. But _parse_confirm_response() in query_model.py expected TWO separate brackets and used `re.search(r'\[gid:([^\]]+)\]')` (anchored to a bracket STARTING with `gid:`) and `re.search(r'\[market:([^\]]+)\]')` (greedy `[^\]]+` to the closing `]`). Against `[market:ML gid:HASH]` the gid regex found no bracket starting with `gid:` → block_gid=None, while the market regex greedily captured `ML gid:HASH` → block_market polluted. Lookup key (None, "ml gid:hash") matched nothing → fall-through to auto-HOLD on all picks. Grading was never corrupted (HOLD = original stake), but no confirm-check adjustment could ever fire. Fix: switched both extractions to token-based regexes that are bracket-agnostic — `re.search(r'gid:\s*([A-Za-z0-9_]+)')` and `re.search(r'market:\s*([A-Za-z]+)')`. These handle all three forms a model might echo: `[market:X gid:Y]`, `[market:X] [gid:Y]`, `[gid:Y market:X]`. Lesson: when a prompt template and its parser are written/edited separately, the echo-back format is a contract — a tolerant token parser beats bracket-delimited regexes because models reproduce the template literally, so any template change silently breaks a strict parser. Second bug found+fixed same pass: {model}_confirm.json's `raw_response` was overwritten each cluster fire (and nulled by the run_lineup_watcher.py auto-HOLD writer), so only the last cluster's raw model output survived — the `picks` array itself was already correctly merged across clusters, so this was an audit-trail/re-parse loss only, not a grading-data loss (no consumer reads raw_response: grade_picks.py and the post-mortem builder read picks/cc_* only). Fix: added an accumulating `raw_responses` list (per-cluster records: checked_at, game_ids, source api|auto_hold, text) in BOTH writers — run_confirm_check() in query_model.py and _write_auto_hold() in run_lineup_watcher.py — keeping scalar `raw_response` as the latest-cluster value for back-compat. Both reads use `.get("raw_responses", [])` so old files are tolerated.
...
2026-06-21: Confirm-check watcher built (5 scripts). Design decisions worth preserving: (1) Batching unit is model+game_id, not model+game_id+market — one AI call handles all of a model's picks on a cluster's games (ML and Total on the same game both in the same call). (2) build_prompt.py --confirm-check must be rebuilt immediately before EVERY cluster fire, not cached per run — the prompt only contains games confirmed at build time, so a cached prompt from an earlier cluster silently omits later-cluster games. (3) Doubleheader collision: [gid:{game_id}] embedded in every ## GAME: header; parser extracts directly with regex rather than matchup-string join, which would silently overwrite one game if two games had identical team-pair strings. (4) Auto-HOLD group-before-fire: when iterating model entries for a game, all market entries for a model must be written before _fired.json is updated — marking chatgpt_{gid} fired after the ML HOLD would skip the Total HOLD on the same game. (5) Guard 5 (slate ceiling) applies to UPGRADE only, not HOLD — HOLD adds no new exposure, so checking already_bet + 0 > ceiling on a HOLD incorrectly fires when Run-1 bets already fill the ceiling. (6) cc_new_units=0 (valid CANCEL outcome) must be checked with `is not None`, not truthiness — `0 or fallback` silently re-stakes a cancelled best bet at original units. (7) Idempotency: grade_picks.py re-reads {model}_confirm.json on every run and re-applies cc fields; because the CANCEL branch continues before any pick mutation except cc_ fields, re-running after a prior grade does not double-count cancels.

---

2026-06-21: Advanced pitcher stats absent for 2026-06-20 and 2026-06-21. context.pitcher_home/away.hard_hit_pct and barrel_pct were null for both dates (advanced_source: None, advanced_fetched_at: None). Jun 19 confirmed working (28 pitchers populated: Gausman 37.7%, Civale 8.2%, Skubal 5.3%, etc.). Two distinct root causes — NOT a pandas-3.x import break or feed failure: (a) Jun 20: manual runs outside run_daily.py default to the hermes-agent venv, which lacks pybaseball — run_daily.py is immune because it hardcodes C:\Users\marko\AppData\Local\Programs\Python\Python312\python.exe; (b) Jun 21 separately: fetch_pitchers.py was never run for that date, so 0 pitcher MLBAM IDs existed in games.json — fetch_pitcher_advanced.py ran but had nothing to enrich. Both are re-runnable execution misses. Fix: (a) always invoke fetch_pitcher_advanced.py via the Python312 interpreter path; (b) confirm fetch_pitchers.py has populated pitcher IDs before running advanced stats. NOTE: run_daily.py already emits a !!!/WARNING banner when any optional step fails — when the banner fires for Pitcher Adv., treat it as a must-fix before firing picks, not a skip — HH%/Brl% affect the AGG pitcher block and the confirm-check watcher comparison. fetch_pitcher_advanced.py ImportError now also prints sys.executable so the wrong-interpreter cause is immediately obvious.

---

2026-06-21: Static data layer expanded — new batted-ball columns added; Exit_Velocity_Barrels.txt dropped. (1) start_pitchers_2yrs_IP5.txt renamed ON DISK to pitchers_season_ld_gb_fb.txt. "2yrs" was unverifiable (IP distribution is consistent with 2026-only; the FanGraphs source was not documented). New loader: load_pitchers_season_batted_ball() reads cols 10/11/12 (LD%/GB%/FB%) from the renamed file; returns fractions (0.241 for 24.1%). load_pitchers_season() and load_stuff_plus() continue to read pitchers_xfip_siera.txt which still has Stuff+ at col 10 — the renamed file does NOT have Stuff+. Do not redirect those loaders; swapping would silently load LD% as Stuff+. (2) team_barrels.txt widened from 4 to 8 columns: cols 4/5/6/7 are GB%/FB%/Pull%/LA°. load_team_barrels() extended to parse all four; cols 4-6 stored as plain floats (39.9, not 0.399) to match barrel_pct/hardhit_pct convention; col 7 (LA, degrees) parsed via _parse_float not _parse_pct. Row guard raised from len<4 to len<8. Rendered in TEAM FORM block as Brl%/HH%/GB%/FB%/Pull%/LA° — BATTER-side rates for the team's offence. (3) Pitcher BB-profile rendered as a distinct segment "BB-profile LD%:x% GB%:x% FB%:x%" in the pitcher one-liner — labelled explicitly to avoid confusion with team_barrels GB%/FB% which are batter-side. Two sources of GB%/FB% exist in the prompt: pitcher-season rates (what batters do against THIS starter) vs team-level rates (how THIS TEAM'S offence hits overall). The labels are distinct by design. (4) Exit_Velocity_Barrels.txt DROPPED. It was a Baseball Savant pitcher leaderboard that duplicated the live fetch in fetch_pitcher_advanced.py (statcast_pitcher_exitvelo_barrels), which already delivers hard_hit_pct and barrel_pct per pitcher into context.pitcher_home/away. Two sources of truth for the same live stat = divergence risk. The live feed wins. If launch_angle_avg or ev50 are ever wanted, extend the existing fetch (no additional API call — those columns already come back in df_ev but are currently discarded). Available extra columns from df_ev: launch_angle_avg, ev_avg, ev50, max_hit_speed, brl_pa, brl_count, bbe, fb_ld_percent.

---

2026-06-21: Weather layer expanded — raw geometry and air-quality fields added to Layer A data. Three new Open-Meteo hourly fields fetched and stored in context.weather: wind_gust_mph (windgusts_10m), humidity_pct (relativehumidity_2m), pressure_hpa (surface_pressure). Four new wind geometry fields added: wind_from_deg (raw met-convention bearing; direction wind blows FROM), wind_to_deg ((from+180)%360; direction wind blows TOWARD), cf_bearing_deg (home-plate→CF azimuth from CF_BEARINGS table), wind_cf_angle_deg (min 0–180° angle between wind-to vector and CF axis). HARD RULE: no derived metrics — no carry index, no weather score, no HR-boost flag. Each model applies its own method. Verified bearings: Wrigley Field (43°) and Coors Field (42°) only. 27 of 30 parks use estimated bearings derived from published wind-effect literature cross-referenced with park layout — not from a primary azimuth source. Sutter Health Park (Athletics) is a placeholder (45°, midpoint of NE quadrant) pending operator measurement. DO NOT use wind_cf_angle_deg as primary totals-staking evidence until the CF_BEARINGS table is backfilled from a published azimuth source (Andrew Clem: andrewclem.com/Baseball or Seamheads: seamheads.com/ballparks). Estimate status is flagged in the prompt itself: cf_bearing 340°(est) / wind-cf angle 65°(est) — verified parks show no tag. The wind_from→wind_to convention: Open-Meteo winddirection_10m is meteorological FROM convention (270° = wind FROM the west = blowing EAST). wind_to_deg is the blowing-TOWARD direction, which is what matters for ballpark carry. A W wind (from_deg=270) → wind_to=90 (blowing toward E/CF at Wrigley). Two build-time warnings emit estimated-park lists: one at fetch_weather.py completion, one at build_prompt.py prompt-build time. Both cite the recommended verification sources.
