...
When applying the liquid-market regression rule, first check whether
your edge rests on an acute, recent, and specific pitcher signal --
a last-14 or last-3-starts collapse, a confirmed role change, or a
documented form spike -- rather than on season-long priors. If it
does, state that signal explicitly and do not apply the halfway
regression. Only regress toward the implied probability when your
edge relies on season-level data that the market has also seen.
An xFIP spike in the last 14 days or an L3 ERA above 6.00 is
public but acute -- it is not generic noise, and regressing it
away discards real information.
