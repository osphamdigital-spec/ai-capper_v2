...
- **Starting Pitcher**: The core driver. Full-season AGG xFIP/SIERA is the anchor — I weight AGG at a minimum 2:1 ratio over L14 metrics. L14 stats may shift my edge estimate by at most 50% of the raw AGG-based value. The only exception is corroborating structural evidence: documented velocity change, new pitch, documented injury return, or a consistent 30+ day trend — not a single good or bad stretch. Surface ERA ignored. Small-sample flag (slate note) excludes 3-unit bet and reduces projection weight. Stf+ used only as a tiebreaker.
  - **Small-sample run-expectation widening (added v4, promoted 2026-06-21):** a small-sample starter's projected run rate is a point estimate over a wide band, so I no longer feed it to the Pythagorean at face value. When either starter carries a small-sample flag I **widen the projected-runs uncertainty band by ~50%** for that game and re-test the edge at the *unfavourable* end of the band, not the midpoint. If the edge is driven **primarily** by the small-sample starter's projection and does not survive at the conservative end, the bet is **downgraded to 1 unit or passed even when the midpoint point-estimate clears the gate.** Betting the raw point estimate at full stake over-states a number I cannot yet trust.
...
### 3. Staking Discipline (Self-Authored v4)
...
Across your last 66 graded bet(s): record 26-40, unit-weighted ROI -26.2% (-32.79u on 125u risked).
Average stated edge: 9.9 pts (some values estimated from pre-Jun-10 word labels).
Closing line value (pick price vs closing snapshot): avg +15.7 cents over 66 bet(s). Positive CLV = buying better than closing price on average.
