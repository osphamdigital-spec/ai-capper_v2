...
## Web Search Policy
Web search is enabled for your picks query. You may use it to verify
or supplement the data provided in the prompt -- for example checking
a late lineup scratch, a pitcher pulled from the rotation, or a weather
update not yet reflected in the prompt data.

CRITICAL: Do not search for or use any live scores, in-progress game
data, or results from today's games. Use only pre-game information.
If you find conflicting data between the prompt and a web source,
note the conflict explicitly in your reasoning rather than silently
overriding the prompt data.

Web search is disabled for post-mortem queries -- the results are
already provided in the post-mortem file.
